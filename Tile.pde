  ...
  void getDisplayFlags(int lw, int lh) {
    ...
    if (type > 0 && type <= 5) {
      if (inBounds(x+1, y, lw, lh)) {
        if (eLevel.tiles[x+1][y][z].type > 1 && eLevel.tiles[x+1][y][z].type <= 4 || (eLevel.tiles[x+1][y][z].type == type && type != 1)) {
          displayFlag++;
        } else if (eLevel.tiles[x+1][y][z].type == 1) {
          displayFlag+=2;
        }
      }
      if (inBounds(x, y+1, lw, lh)) {
        if (eLevel.tiles[x][y+1][z].type > 1 && eLevel.tiles[x][y+1][z].type <= 4 || (eLevel.tiles[x][y+1][z].type == type && type != 1)) {
          displayFlag+=3;
        } else if (eLevel.tiles[x][y+1][z].type == 1) {
          displayFlag+=6;
        }
      }
      if (inBounds(x-1, y, lw, lh)) {
